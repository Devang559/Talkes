...
export const useBleMesh = ({
  ...
  const connectedPeersRef = useRef<
    Map<string, { deviceId: string; name: string }>
  >(new Map());
  const messageBufferRef = useRef<Map<string, BleMessagePayload[]>>(
    new Map()
  );
  ...
  const requestPermissions = useCallback(async (): Promise<boolean> => {
    ...
    if (Platform.OS === 'android') {
      ...
      if (allGranted) {
        ...
        const poweredOn =
          (newState as State) === State.PoweredOn;
  ...
  const parseScanData = (
    device: Device
  ): { talkesId: string; name: string } | null => {
    try {
      /*
       * Compact BLE discovery payload:
       * Base64 -> UTF-8 -> Talkes ID
       *
       * Example:
       * "VEstMTIzNDU=" -> "TK-12345"
       */
      if (device.serviceData) {
        const serviceData = device.serviceData as Record<
          string,
          string
        >;
        ...
        for (const [key, value] of Object.entries(serviceData)) {
          ...
          try {
            ...
            const discoveredTalkesId = decoded.trim();

            if (
              discoveredTalkesId &&
              discoveredTalkesId.startsWith('TK-')
            ) {
              const name = device.localName || 'Talkes';

              return {
                talkesId: discoveredTalkesId,
                name,
              };
            }
          } catch {
      ...
      /*
       * Legacy manufacturer-data fallback.
       * Kept here so older Talkes builds can still be discovered.
       */
      if (device.manufacturerData) {
        try {
          ...
          // Ignore invalid manufacturer data.
        }
      }

      /*
       * Final fallback to Bluetooth local name.
       */
      if (device.localName) {
        return {
          talkesId: device.localName,
          ...
          manager.startDeviceScan(
            ...
            (
              ...
              if (
                !decoded ||
                !decoded.talkesId ||
                decoded.talkesId === talkesId
              ) {
                return;
              }

              peerHandleToDeviceIdMapRef.current.set(
                decoded.talkesId,
                device.id
              );
              ...
              setKnownPeers(prevPeers => {
                ...
                const peer: Peer = {
                  ...
                  avatar: getAvatarUri(
                    getAvatarIndexForId(decoded.talkesId)
                  ),
                  signalColor: getRandomSignalColor(),
                  status: connectedPeersRef.current.has(
                    decoded.talkesId
                  )
                    ? 'active'
                    : 'connect',
    ...
    (base64Data: string) => {
      try {
        ...
        if (
          ...
          messageBufferRef.current.delete(
            payload.messageId
          );
  ...
  const handleIncomingMessageFromAdvertisedDevice =
    useCallback(
      (message: string) => {
        try {
          const payload: BleMessagePayload =
          ...
          console.error(
            'BLE native message decode error:',
            err
          );
        }
      },
  ...
  const connectToDevice = useCallback(
    async (peer: Peer) => {
      ...
      const deviceId =
        peerHandleToDeviceIdMapRef.current.get(
          peer.id
        );
      ...
      try {
        ...
        device.onDisconnected(() => {
          connectedPeersRef.current.delete(
            peer.id
          );

          peerHandleToDeviceIdMapRef.current.delete(
            peer.id
          );
          ...
          setKnownPeers(prev =>
            prev.map(existingPeer =>
              existingPeer.id === peer.id
                ? {
                    ...existingPeer,
                    status: 'connect',
                  }
        ...
        const mtuResult =
          await device.requestMTU(MAX_MTU);
        ...
        if (
          monitorSub &&
          typeof monitorSub.remove === 'function'
        ) {
          subscriptionsRef.current.push(
            monitorSub
          );
  ...
  const sendMessage = useCallback(
    async (
      ...
      if (!deviceId) {
        console.warn(
          'BLE: Cannot send message - not connected to',
          peerHandle
        );
      ...
      const device = (
        await manager.devices([deviceId])
      ).find(
        currentDevice =>
    ...
    if (
      ...
      const subscription =
        eventEmitter.addListener(
          'BleMessageReceived',
          (event: any) => {
            handleIncomingMessageFromAdvertisedDevice(
              event.message
            );
          }
        );
